                            ...
                            <table class="col-lg-12 table table-bordered mt-4">
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td><label for=""><input type="checkbox" id="selectall"> Select all</label>
                                        </td>
                                    </tr>
                                    @foreach ($rights as $right)
                                        <tr>
                                            <td><span class="pl-2"><b
                                                        class="text-capitalize">{{ $right->module }}:</b></span></td>
                                            <td>
                                                @foreach ($right->rights as $row)
                                                    <label for="" class="role-label"><input type="checkbox"
                                                            class="permission-check"
                                                            name="permission[{{ $row->module }}][{{ $row->id }}]"
                                                            value="1"> {{ explode('.', $row->name)[1] }} </label>
                                                @endforeach
                                            </td>
                                        </tr>
                                    @endforeach
